...
TEST_P(ParserRegexTest, parseQuery)
{
    ...
    DB::ASTPtr ast;
    ASSERT_NO_THROW(ast = parseQuery(*parser, input_text.data(), input_text.data() + input_text.size(), 0, 0, 0));
...
TEST_P(ParserKQLTest, parseKQLQuery)
{
    ...
    if (expected_ast)
    {
        ...
        {
            EXPECT_THROW(parseKQLQuery(*parser, input_text.data(), input_text.data() + input_text.size(), 0, 0, 0), DB::Exception);
        }
        else
        {
            DB::ASTPtr ast;
            ASSERT_NO_THROW(ast = parseKQLQuery(*parser, input_text.data(), input_text.data() + input_text.size(), 0, 0, 0));
    ...
    {
        ASSERT_THROW(parseKQLQuery(*parser, input_text.data(), input_text.data() + input_text.size(), 0, 0, 0), DB::Exception);
    }
}
